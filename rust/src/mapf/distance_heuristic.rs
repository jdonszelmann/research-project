...
impl<A, E, ET: EdgeType, G: GraphLike<(A, A), E, ET>> DistanceHeuristic<(A, A), E, ET, G>
    for Manhattan
where
    A: Copy + Clone + num::Num + Ord + num::ToPrimitive,
{
    fn heuristic(graph: G, from: Location, to: Location) -> f64 {
        let (x1, y1) = *graph
            .graph()
...
#[cfg(test)]
mod tests {
    use crate::mapf::distance_heuristic::{DistanceHeuristic, Euclidean, Manhattan};
